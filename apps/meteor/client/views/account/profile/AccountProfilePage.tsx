...
const AccountProfilePage = (): ReactElement => {
  ...
  const logoutOtherClients = useEndpoint(
    "POST",
  ...
  const allowUserStatusMessageChange = useSetting(
  ...
  const allowOAuthPasswordChange = useSetting(
        ...
        await saveFn(
          {
            ...
            ...(allowEmailChange && user
              ? getUserEmailAddress(user) !== email && { email }
              : {}),
    ...
    (passwordOrUsername, shouldChangeOwner, shouldBeRemoved) => {
      ...
      return setModal(() => (
        <ConfirmOwnerChangeModal
          ...
          onCancel={closeModal}
          contentTitle={t(
            `Delete_User_Warning_${erasureType}` as TranslationKey
          )}
    ...
    const handleConfirm = async (passwordOrUsername: string): Promise<void> => {
      try {
        ...
        if (error.error === "user-last-owner") {
          ...
          return handleConfirmOwnerChange(
            passwordOrUsername,
            shouldChangeOwner,
            shouldBeRemoved
          );
