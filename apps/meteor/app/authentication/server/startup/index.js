...
Accounts.onCreateUser(function (options, user = {}) {
  ...
  Promise.await(
    Apps.triggerEvent(AppEvents.IPostUserCreated, {
      user,
...
Accounts.insertUserDoc = _.wrap(
  ...
  function (insertUserDoc, options, user) {
    ...
    if (user.services && !user.services.password) {
      const defaultAuthServiceRoles = parseCSV(
        settings.get(
    ...
    if (user.username) {
      ...
      if (settings.get("Accounts_SetDefaultAvatar") === true) {
        const avatarSuggestions = Promise.await(
...
Accounts.validateLoginAttempt(function (login) {
  ...
  if (
    !Promise.await(isValidLoginAttemptByIp(getClientAddress(login.connection)))
  ) {
...
export const MAX_RESUME_LOGIN_TOKENS =
  parseInt(process.env.MAX_RESUME_LOGIN_TOKENS) || 50;
